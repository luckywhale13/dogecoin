...
Verify(const CScript& scriptSig, const CScript& scriptPubKey, bool fStrict, ScriptError& err)
{
    ...
    return VerifyScript(scriptSig, scriptPubKey, NULL, fStrict ? SCRIPT_VERIFY_P2SH : SCRIPT_VERIFY_NONE, MutableTransactionSignatureChecker(CHAINSIG_VERSION_LATEST, &txTo, 0, txFrom.vout[0].nValue), &err);
}
...
BOOST_AUTO_TEST_CASE(sign)
{
    ...
    for (int i = 0; i < 8; i++) {
        ...
        for (int j = 0; j < 8; j++)
        {
            ...
            bool sigOK = CScriptCheck(CCoins(txFrom, 0), txTo[i], 0, SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_STRICTENC, false, &txdata, CHAINSIG_VERSION_LATEST)();
            if (i == j)
...
BOOST_AUTO_TEST_CASE(AreInputsStandard)
{
    ...
    BOOST_CHECK(SignSignature(CHAINSIG_VERSION_LATEST, keystore, txFrom, txTo, 0, SIGHASH_ALL));
    BOOST_CHECK(SignSignature(CHAINSIG_VERSION_LATEST, keystore, txFrom, txTo, 1, SIGHASH_ALL));
    BOOST_CHECK(SignSignature(CHAINSIG_VERSION_LATEST, keystore, txFrom, txTo, 2, SIGHASH_ALL));
