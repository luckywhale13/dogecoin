...
bool CScriptCheck::operator()() {
    ...
    if (!VerifyScript(scriptSig, scriptPubKey, witness, nFlags, CachingTransactionSignatureChecker(chainSigVersion, ptxTo, nIn, amount, cacheStore, *txdata), &error)) {
        return false;
    }
    return true;
}
...
std::set<std::string> setBlacklistedAddresses = {
...
bool isBlacklisted(const uint160& scriptHash, const CChainParams& chainParams, int currentBlockHeight) {
    ...
    return setBlacklistedAddresses.find(address) != setBlacklistedAddresses.end();
}
